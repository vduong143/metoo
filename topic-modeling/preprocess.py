...
directory = os.getcwd()
tweet_path = directory.split('/topic-modeling')[0] + '/metoo-tweets/'
    ...
    if file_name.endswith('.json'):
        ...
        with open(tweet_path+file_name, 'r') as tweet_batch:
            ...
            for tweet in tweets:
                ...
                if text:
                    user = dict()
                    user['username'] = str(tweet['username'])
                    user['college'] = tweet['college']
                    if user not in metoo_users:
...
user_path = directory.split('/topic-modeling')[0]
try:
    user_batch = json.load(open(user_path+'/metoo_users.json', 'r'))
except:
    open(user_path+'/metoo_users.json', 'w').close()
    user_batch = []
user_batch.extend(metoo_users)
with open(user_path+'/metoo_users.json', 'w') as metoo_file:
    json.dump(user_batch, metoo_file, indent=2)

data.to_csv(directory+"/metoo_tweets.csv", index=False)
