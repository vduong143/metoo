...
# import data
data = pd.read_csv(directory+"/metoo_tweets.csv")
...
def get_topic_documents(all_documents, W, topic_index):
    # reverse sort the values to sort the indices
    indices = np.argsort(W[:,topic_index])[::-1]
    # get the documents corresponding to the top-ranked indices
    documents = []
    for doc_index in indices:
        documents.append(all_documents[doc_index])
    return documents

for topic in range(k):
    topic_documents = get_topic_documents(raw_documents, W, topic)
    with open(directory+"/clusters/topic_{}.txt".format(topic), "w") as f:
        for d in topic_documents:
